...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName==='review') {
    ...
    if (!db.trivia[uid]) db.trivia[uid]={points:0,correct:0};
    db.trivia[uid].points+=3; // +3 نقاط للمراجعة
    const reviewCh=guild.channels.cache.find(c=>c.name.includes('review-center'));
    const embed=new EmbedBuilder().setColor(0xF1C40F).setTitle(`⭐ مراجعة: ${title}`).setDescription(review).addFields(
      {name:'التقييم',value:`${stars} ${rating}/10`,inline:true},
  ...
  if (commandName==='sprint') {
    ...
    setTimeout(async()=>{
      ...
      for (let i=0;i<Math.min(parts.length,3);i++) {
        const [wid]=parts[i];
        if (!db.trivia[wid]) db.trivia[wid]={points:0,correct:0};
        db.trivia[wid].points+=podiumPts[i];
      }
  ...
  if (commandName==='quote') {
    const q=getRandom(QUOTES);
    return interaction.reply({embeds:[new EmbedBuilder().setColor(0xFFD700).setTitle('💬 اقتباس أدبي').setDescription(`*"${q.text}"*`).addFields({name:'✍️',value:`**${q.author}**`})]});
  }
  if (commandName==='fact') {
  ...
  if (commandName==='versus') {
    ...
    const msg=await interaction.reply({embeds:[new EmbedBuilder().setColor(0xE74C3C).setTitle('⚔️ المعركة الأدبية!')
      .setDescription(`**${op1}** ضد **${op2}**\n\nصوّت بـ 1️⃣ أو 2️⃣`)
      .addFields({name:'1️⃣',value:op1,inline:true},{name:'⚔️',value:'VS',inline:true},{name:'2️⃣',value:op2,inline:true})
  ...
  if (commandName==='serverstats') {
    ...
    return interaction.reply({embeds:[new EmbedBuilder().setColor(0x5865F2).setTitle(`📊 ${g.name}`).setThumbnail(g.iconURL()).addFields(
      {name:'👥 الأعضاء',value:`${g.memberCount-bots}`,inline:true},{name:'🤖 بوتات',value:`${bots}`,inline:true},
      {name:'📁 قنوات',value:`${g.channels.cache.size}`,inline:true},{name:'🎭 رتب',value:`${g.roles.cache.size}`,inline:true},
  ...
  if (commandName==='quiz') {
    ...
    await interaction.reply({embeds:[new EmbedBuilder().setColor(0xF39C12).setTitle('🎮 سؤال تنافسي!')
      .setDescription(`**${q.q}**`)
      .addFields({name:'⭐ النقاط',value:`${q.pts} نقطة`,inline:true},{name:'🎯 الصعوبة',value:q.diff,inline:true})
  ...
  if (commandName==='pts') {
    ...
    return interaction.reply({embeds:[new EmbedBuilder().setColor(0xF1C40F).setTitle(`🏅 نقاط ${user.displayName}`)
      .addFields(
        {name:'⭐ إجمالي النقاط',value:`${pts.points}`,inline:true},
        {name:'✅ إجابات صح',value:`${pts.correct}`,inline:true},
        {name:'🏆 الترتيب',value:rank>0?`توب ${rank}`:'-',inline:true},
        {name:'💎 المستوى',value:tier,inline:false},
      )],ephemeral:true});
  }
  if (commandName==='top') {
    ...
    return interaction.reply({embeds:[new EmbedBuilder().setColor(0xFFD700).setTitle('🏆 التصنيف التنافسي')
      .setDescription(sorted.map(([id,d],i)=>{
        const medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}.`;
        return `${medal} <@${id}> — **${d.points} نقطة**`;
      }).join('\n'))
      .addFields({name:'━━━━━━━━━━━━',value:`ترتيبك: **توب ${myRank||'?'}** | نقاطك: **${myPts}**`,inline:false})
      .setFooter({text:`${SERVER_NAME} • Competitive Rankings`})]});
  }
  ...
  if (commandName==='readingchallenge') {
    ...
    return interaction.reply({embeds:[new EmbedBuilder().setColor(0x57F287).setTitle('📚 تم التسجيل في تحدي القراءة!')
      .setDescription(`مرحباً ${user}!\nسجّلت في **تحدي القراءة الشهري** 🎉\n\nاستخدم \`/addbook\` لتسجيل كتبك وتجمع نقاط!\n*في نهاية الشهر يُعلن الفائز ويحصل على جائزة!*`)
      .setFooter({text:`${SERVER_NAME} • Reading Challenge`})]});
  }
  if (commandName==='bookclub') {
    return interaction.reply({embeds:[new EmbedBuilder().setColor(0x5865F2).setTitle('📖 نادي الكتاب الأسبوعي')
      .setDescription([
        '**كيف يعمل نادي الكتاب؟**',
        '',
        '📅 كل أسبوع نختار كتاباً أو رواية للقراءة الجماعية',
        '💬 نناقشها في قنوات الديسكورد',
        '⭐ من أنهاها يحصل على **+10 نقاط** ومراجعة منظمة',
        '',
        '📚 للمشاركة: اقرأ الكتاب المحدد واكتب مراجعتك بـ `/review`',
        '',
        '*تابع `#book-of-the-month` لمعرفة الكتاب الحالي!*',
      ].join('\n'))
  ...
  if (commandName==='authorprofile') {
    ...
    if (!db.trivia[uid]) db.trivia[uid]={points:0,correct:0};
    db.trivia[uid].points+=5; // +5 نقاط لنشر رواية
    const pubCh=guild.channels.cache.find(c=>c.name.includes('original novels')||c.name.includes('author'));
    const embed=new EmbedBuilder().setColor(0x9B59B6).setTitle(`📖 ${title}`).setDescription(desc)
      .addFields({name:'🏷️ النوع',value:genre,inline:true},{name:'📊 الحالة',value:statusLabel[status],inline:true},{name:'✍️ المؤلف',value:`${user}`,inline:true})
      .setTimestamp();
  ...
  if (commandName==='warn') {
    ...
    try { await target.send({embeds:[new EmbedBuilder().setColor(0xFEA500).setTitle('⚠️ تلقيت تحذيراً')
      .addFields({name:'📋 السبب',value:reason},{name:'👮 بواسطة',value:user.tag},{name:'⚠️ عدد تحذيراتك',value:`${count}`})
      .setDescription(count>=3?'🔴 وصلت للحد الأقصى — سيُطبق تايم أوت تلقائياً!':'')
      .setFooter({text:GUILD_NAME})]}); } catch {}
  ...
  if (commandName==='warnings') {
    ...
    return interaction.reply({embeds:[new EmbedBuilder().setColor(warns.length>=3?0xED4245:0xFEA500).setTitle(`⚠️ تحذيرات ${target.user.tag}`)
      .setDescription(warns.length===0?'✅ لا يوجد تحذيرات':warns.map((w,i)=>`**${i+1}.** ${w.reason}\n> 👮 ${w.by} | 🕒 ${new Date(w.time).toLocaleDateString('ar-SA')}`).join('\n\n'))
  ...
  if (commandName==='monthlyawards') {
    ...
    const embed=new EmbedBuilder().setColor(0xFFD700).setTitle(`🏆 جوائز ${new Date().toLocaleDateString('ar-SA',{month:'long',year:'numeric'})}`)
...
client.on('messageCreate', async (message) => {
  ...
  await message.reply({embeds:[new EmbedBuilder().setColor(0x57F287).setTitle('🎉 إجابة صحيحة!')
    .setDescription(`${message.author} حصل على **+${pts} نقطة**!\nإجمالي نقاطه: **${db.trivia[uid].points}**`)
    .setFooter({text:'استخدم /pts لترى نقاطك الكاملة'})]});
